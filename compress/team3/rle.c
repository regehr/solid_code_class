...
void rle_encode(FILE *input, FILE *output)
{
    ...
    while (true)
    {
        /* Read out a byte. If the number of bytes read isn't 1, then we're at
           ...
           error. */
        char byte;
        size_t num_read = xfread(&byte, 1, 1, input);
        if (num_read != 1) break;
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            if (encoding_length != MAX_RUN_LENGTH) continue;
