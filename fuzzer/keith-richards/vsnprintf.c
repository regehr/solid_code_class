...
static size_t sn_write(MUSL_FILE *f, const unsigned char *s, size_t l)
{
...
int musl_vsnprintf(char *restrict s, size_t n, const char *restrict fmt, va_list ap)
{
	int r;
	char b;
	MUSL_FILE f = { .lbf = EOF, .write = sn_write, .lock = -1 };
