...
static void out(MUSL_FILE *f, const char *s, size_t l)
{
	__fwritex((void *)s, l, f);
}

static void pad(MUSL_FILE *f, char c, int w, int l, int fl)
{
	char pad[256];
...
static int fmt_fp(MUSL_FILE *f, long double y, int w, int p, int fl, int t)
{
...
static int printf_core(MUSL_FILE *f, const char *fmt, va_list *ap, union arg *nl_arg, int *nl_type)
{
	char *a, *z, *s=(char *)fmt;
	unsigned l10n=0, fl;
	int w, p;
	...
	for (;;) {
		...
		case 's':
			a = arg.p ? arg.p : "(null)";
			/* This is so hacky you don't even know */
